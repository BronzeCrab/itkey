...
for port_id in $(echo $ports_id_list | grep -Po '(?<="id":\s")[a-z0-9-]*'); do
	...
	if [[ `echo $?` -eq 0 ]]; then
		...
		# it is possible to figure out what router is serving that port
		echo "Port $port_id wasn't deleted, something went wrong" 
		...
		fixed_ips=$(neutron port-show -f value -F fixed_ips $port_id)
